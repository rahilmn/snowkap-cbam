-- ============================================================
-- Snowkap CBAM
-- P13 release-blocker remediation, ITERATION 7: self-discovered via
-- live browser end-to-end verification of the R7 clause 2 / R9
-- regulatory resolver fix (src/domain/regulatory/resolve-default-value.ts,
-- commit 6094593) -- NOT caught by that fix's own domain-level tests,
-- pnpm typecheck, or pnpm regulatory:verify, all of which passed clean.
--
-- The R7/R9 fix makes a previously-impossible combination reachable: a
-- listed/MAPPED country's own regulatory record can now be UNAVAILABLE
-- and fall back to the "Other Countries and Territories" table, so a
-- DEFAULT determination can carry country_mapping.status = 'MAPPED'
-- (naming the originally-requested, real, listed country) together with
-- reason = 'OTHER_COUNTRIES_FALLBACK' and a record_identity whose own
-- origin_country_name is '_Other Countries and Territorie' (the ACTUAL
-- matched row, not the requested country's).
--
-- 20260829610000's validator (v6, unchanged by this migration's
-- predecessors back through v2) never anticipated this combination: its
-- very first identity check,
--   `if v_origin_country_name is distinct from
--    (v_resolution->'country_mapping'->>'regulatory_country_name')`,
-- is unconditional, and rejects this new, legitimate case outright --
-- record_identity.origin_country_name ("_Other Countries and
-- Territorie") is never equal to country_mapping.regulatory_country_name
-- (the real requested country's name, e.g. "India") in this case, by
-- design. Live-reproduced end-to-end through the real UI before being
-- fixed here: a shipment line (India, TARIC 2507008080 -- a real
-- UNAVAILABLE row in the ACTIVE dataset with a genuine
-- Other-Countries-and-Territories fallback value) resolved correctly at
-- the domain layer, but the UPDATE persisting that determination was
-- rejected by this trigger with a raw 42501, surfaced to the user as
-- "This shipment is locked or void and can no longer be edited." -- a
-- materially misleading message for what was actually a validation
-- rejection, not a lock/void state.
--
-- Fixed: a new, explicit branch for MAPPED + OTHER_COUNTRIES_FALLBACK
-- that validates the two halves of this claim separately -- (a) is the
-- claimed country_mapping.regulatory_country_name a REAL, ISO-verified
-- name (never fabricated), and (b) does the ACTUAL matched record
-- genuinely come from the Other Countries and Territories table. Every
-- other combination (exact matches, UNLISTED + fallback) keeps its
-- exact prior validation logic unchanged, in the same order, with the
-- same strictness -- this is additive, not a broadening of the existing
-- checks.
-- ============================================================
...
begin
    ...
    -- NEW in this iteration: a listed/MAPPED country whose own record is
    -- UNAVAILABLE can now resolve via the Other Countries and
    -- Territories fallback (R7 clause 2 / R9) -- the ACTUAL matched
    -- record's own origin is genuinely the fallback territory's, not
    -- the requested country's, so the two identity checks below are
    -- validated against DIFFERENT things than the else branch: the
    -- claimed regulatory_country_name still against a real ISO lookup
    -- (never fabricated), and the matched record's own origin against
    -- the fallback sentinel itself (proving the value really came from
    -- that table, not an invented one).
    if (v_resolution->'country_mapping'->>'status') = 'MAPPED'
    ...
    then
        ...
        -- Unchanged from v6: every other combination (exact matches,
        -- and UNLISTED + fallback) keeps its exact prior validation.
...
comment on function app.emission_determination_matches_regulatory_record(jsonb, uuid, text, text, text) is
    '2026-08-30 (P13 review iteration 7): adds a dedicated branch for '
    'MAPPED country_mapping + OTHER_COUNTRIES_FALLBACK reason, the '
    'combination the R7 clause 2 / R9 resolver fix (commit 6094593) made '
    'newly reachable -- a listed country whose own record is UNAVAILABLE '
    'can now genuinely resolve via the Other Countries and Territories '
    'table. Self-discovered via live browser end-to-end verification, '
    'not by any prior test in this migration series. Every other '
    'combination keeps v6''s exact prior validation logic unchanged.';
