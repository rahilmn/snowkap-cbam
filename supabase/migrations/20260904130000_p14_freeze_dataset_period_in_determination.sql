-- ============================================================
-- Snowkap CBAM
-- P14 (2026-09-04), owner decision 7: freeze the emissions-data period
-- into the calculation's frozen determination.
--
-- The ACTUAL snapshot already froze WHICH record it used
-- (emission_data_id) and which version of it. It did not freeze what
-- that record was a measurement OF.
--
-- Those look interchangeable while you are looking at live data,
-- because an installation has one ACTIVE record per period -- and they
-- stop being interchangeable the moment the record is superseded,
-- discarded, or read across a sharing grant that is later revoked. A
-- frozen calculation then cannot answer "emissions data for which
-- period?" without a live read, and the live read is precisely what may
-- no longer be permitted or may now answer differently. Everything else
-- in this snapshot is frozen for that reason; the period was the
-- omission.
--
-- WHAT THIS IS NOT. It is not a rule about whether a dataset period has
-- to match the shipment's reporting period. That question is genuinely
-- open, is recorded as an owner decision, and is not answered here --
-- inventing an answer inside a consistency check is how a regulatory
-- position gets made by accident. This freezes provenance so the
-- question can be answered later, about historical calculations,
-- truthfully.
--
-- The check mirrors record_provenance's (20260903120000) exactly:
-- required for new writes, compared against the emission_data row's own
-- columns, and never revalidated on rows this trigger is not rewriting
-- -- so determinations frozen before the field existed are unaffected.
--
-- The body below is the current function verbatim with one clause
-- inserted beside the record_provenance check. Nothing else changed.
-- ============================================================
...
begin
    ...
    if v_method = 'ACTUAL' then
        ...
        -- ------------------------------------------------------------
        -- 2026-09-04 (owner decision 7). The dataset period is frozen
        -- into the snapshot, so it is checked here rather than trusted
        -- from the writer -- exactly as record_provenance above is, and
        -- for exactly the same reason: an unchecked frozen field is a
        -- decorative one, and a member posting raw PostgREST could put
        -- any period on a determination taken from any record.
        --
        -- This validates PROVENANCE, not policy. It asserts only that
        -- the period the snapshot claims is the period the emission
        -- record actually covers. It makes no claim about whether that
        -- period must equal the shipment's -- that question is open and
        -- belongs to the owner, and answering it here would smuggle a
        -- regulatory rule into a consistency check.
        --
        -- Presence is REQUIRED for new writes, like record_provenance:
        -- determinations frozen before this field existed carry no such
        -- key, and this trigger never revalidates a row it is not
        -- rewriting, so they are unaffected.
        -- ------------------------------------------------------------
        ...
        -- QUARTERLY carries a quarter; ANNUAL must not invent one. Both
        -- directions matter, so this compares the nullability too.
