-- ============================================================
-- Snowkap CBAM
-- P14 remediation (2026-09-04): the filing checks cannot be raced.
--
-- 20260905120000 made filing verify that the population it files is the
-- population approved. That check is made near the top of the function
-- and the snapshot is aggregated near the bottom, and only the
-- declaration row was locked -- so a line deleted between the two would
-- have produced a snapshot the check had already approved.
--
-- Reachable only by an administrator or an unbound role, since editing a
-- line requires reopening its shipment, which 20260905110000 made
-- administrative. Closed anyway: a check that can be raced is a check
-- with a footnote, and "materially different filing" is not a footnote.
--
-- Regenerated in full from the live definition rather than patched.
-- ============================================================
