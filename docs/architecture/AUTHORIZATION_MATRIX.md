...
**Known, disclosed staleness (confirmed and found worse, 2026-08-30, during
the P13 final non-blocked-work audit)**: this document's own file:line
...
citations into `organizations-isolation.test.ts` are now off by 500-600+
lines (that file has grown substantially since its citations were last
...
This is a maintenance/DX gap, not a live authorization vulnerability — the
...
re-audited or found wrong in this pass, only the line numbers pointing at
where to find the proof. Treat every citation in this document as
"approximately here, re-grep to confirm the exact line" until a future
pass regrounds them all — this document's own opening rule already says
as much ("if a row here stops matching `grep`, the row is wrong, not the
code"), but the line-number staleness makes that regrounding step
necessary far more often than a reader would expect from a document that
otherwise reads as freshly verified.
