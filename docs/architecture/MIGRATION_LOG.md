...
| `20260831110000` | `p13_review_shipment_line_determination_forgery_fix_v8.sql` | **Final adversarial review, HIGH — emission understatement.** The determination validator enforced the SHAPE of an `OTHER_COUNTRIES_FALLBACK` claim but never R7 clause 2's PRECONDITION. Live-reproduced: a China / 2804 10 00 line (own default 26.640 AVAILABLE) was ACCEPTED claiming the Other-Countries value 17.740 — a 33% under-report presented downstream as a validator-approved figure. Reachable on BOTH the v7 MAPPED branch and the pre-existing UNLISTED branch, so live before v7 too. Now requires that no AVAILABLE own-country value exists for the same code and route. Verified both directions: attack rejected, legitimate India / 2507008080 fallback still accepted. |
| `20260831120000` | `p13_review_sharing_grant_status_and_calculation_numeric_hardening.sql` | **Final adversarial review — three findings.** (HIGH) `sharing_grants_insert_own_org` did not constrain `status`, so an ADMIN/OWNER could mint an ACTIVE grant naming any victim org as grantee and read that org's full row (`eori_number`, `cbam_declarant_status`, slug, country) via `organizations_select_via_own_issued_sharing_grant` — with no acceptance and no notice; now requires `status = 'INVITED'` (safe: `issueSharingGrant` never sets status, relying on the INVITED default). (MEDIUM) that same SELECT policy omitted `expires_at`, so a lapsed-but-still-ACTIVE grant disclosed indefinitely; now applies the same predicate as `app.user_shared_installation_ids()`. (HIGH) `calculation_results` had no constraint on its DecimalString numeric columns, so a forged `'NaN'` propagated into the period total; now pinned to canonical decimal form reusing 20260829440000's existing regex. |

| `20260831140000` | `p13_review_audit_events_payload_bounds.sql` | **P13 remaining-findings review — unbounded append-only growth.** `audit_events` had no bound on `payload`, `aggregate_id` or `correlation_id`: six constraints existed, none of them touching those columns, and no application-side size guard existed either. Because the table is deliberately append-only (no UPDATE and no DELETE policy, by design — §21 immutable history), anything an authenticated member wrote via a direct PostgREST insert with a valid catalog `event_type` was also *unremovable*. Adds an object-shape check plus 8 KiB / 200 / 200 bounds. Uses `octet_length(payload::text)` and NOT `pg_column_size`, which reports the TOAST-compressed size and would let highly compressible filler past. Bounds validated against real data before writing: max payload 390 bytes across 27 production rows (and 380 across 1,024 local rows), so every existing row validates with no NOT VALID dance. The `event_type` half of this finding was verified as ALREADY closed by `20260829430000` and deliberately left untouched. |

| `20260902090000` | `p13_review_shipment_line_determination_validator_v9.sql` | **B1/B2 semantics, decided in `docs/regulatory/DETERMINATION_VALIDATOR_SEMANTICS_DECISION_MEMO.md`.** B1: the validator compared the matched record's route to the line's declared route by string equality, which rejected every legitimate unique route-specific resolution -- 6,487 (country, code) pairs including 100% of aluminium, and v6's own attack fixture among them. Replaced by re-deriving the resolver's own uniqueness rule (R6/R10). B2: the UNLISTED arm compared against `regulatory_country_name`, which is intentionally absent for UNLISTED, so `IS DISTINCT FROM NULL` rejected every UNLISTED determination; replaced by three checkable invariants (origin genuinely absent from `countries`, record is the Other-Countries row, reason is `OTHER_COUNTRIES_FALLBACK`). **This migration also shipped a CRITICAL regression -- see `20260902140000`.** |
| `20260902140000` | `p13_review_shipment_line_determination_validator_v10.sql` | **CRITICAL -- closes an understatement forgery that v9 introduced.** v9's relaxed route binding fires only when both the declared route and the claimed record's route are non-null, so it stopped rejecting claims whose record has `route = NULL`; meanwhile R7 clause 2's precondition still filtered the own country by exact route equality, so it missed the origin country's usable value. Together they re-opened the Other-Countries understatement forgery across **954** combinations, worst case **-68.5%** (India / 7203, 4.200 -> 1.325), which v8 had blocked. v9 was live in production with that regression for about an hour; verified zero data impact (no determination was written in the window). v10 makes the precondition use the same route filter as the uniqueness rule. 205 probes across the population accept zero. Full account: `P13_RELEASE_READINESS_REPORT.md` §55. |

| `20260902150000` | `p14_sharing_counterparty_org_names_terminal_grants.sql` | **Live-production provenance defect.** `20260831100000` returned counterparty names only for a currently-ACTIVE, unexpired grant, so the moment grant 942ba281 (ABC -> SNOWKAP) was revoked at 15:28:55 on 2026-09-02 the importer /emissions overview rendered "Shared by Unknown organization" as the source of a frozen ACTUAL determination it had already calculated (2 tCO2e) and could file -- defeating, one step later in SQL, the very reason `list-actual-determined-lines.ts` resolves the grantor through the grant row. Widens direction 1 (a GRANTEE resolving its GRANTOR) to any status, which is self-disclosure by the grantor; leaves direction 2 (a GRANTOR resolving its GRANTEE) at live-ACTIVE or provably-accepted-bootstrap, so a self-issued, self-revoked sham direct grant still names nobody (the 20260829320000 attack class). Still returns only (id, name); `organizations` SELECT RLS is untouched. Also adds a BEFORE INSERT trigger enforcing the bootstrap XOR that direction 2 acceptance proof depends on -- until now enforced only by an RLS INSERT policy that service_role, SECURITY DEFINER contexts, seeds and migrations all bypass, while the table CHECK is an OR. Deliberately and partially narrows 20260831120000 section 2 and 20260829390000 sections 1-2, both of which were defending the FULL row rather than the name. |

| `20260903100000` | `p14_accept_sharing_grant_requires_importer_capability.sql` | **Unauthorized data access.** `accept_sharing_grant_invitation` verified an authenticated session, a confirmed email matching `invited_email`, an INVITED and unexpired grant, that the target is not the grantor, and active membership of `p_org_id` -- never that `p_org_id` holds `IMPORTER_DECLARANT`. `p_org_id` comes from the application as the cookie-derived ACTIVE organization, so a user belonging to more than one non-grantor org bound the producer's data to whichever one the cookie named. The binding is IMMUTABLE (`app.prevent_sharing_grant_fact_change` permits `grantee_org_id` to change exactly once, from NULL) and admits every member of that org to the producer's ACTIVE, VERIFIED data. Returns `CAPABILITY_NOT_HELD`. Capabilities are append-only, so an org cannot lose the capability and strand a live grant. Paired with an application-layer guard and an explicit org chooser on `/accept-invitation`. |
| `20260903110000` | `p14_filing_requires_calculation_quantity_to_match_line.sql` | **Filed-declaration understatement.** `calculation_results` is member-insertable by design and its CHECK pins the FORM of a decimal string, never its magnitude, so a member posting raw PostgREST could insert a row for their own line carrying that line's exact current determination with `quantity` `'1'` against a line declaring 1000 tonnes. Every existing wall passes it: the determination comparison matches (the determination genuinely IS the line's), and `reproduceCalculationResult` recomputes from the row's OWN inputs so a self-consistent row reproduces perfectly -- a reproduction check can prove a number follows from the inputs beside it, never that those inputs were the line's. `record_declaration_filed` now folds a quantity or basis mismatch into the same INCOMPLETE path a missing calculation uses. Text comparison, because byte-equality is already the reproduction contract and `::numeric` would accept `'1000.0'` for `'1000'`. Body carried across verbatim from `20260829470000`. |
| `20260903120000` | `p14_d2_importer_entered_provenance.sql` | **Owner decision D2.** `operators.provenance` and `installations.provenance` have held `IMPORTER_ENTERED` since `20260829220000`; nothing was ever built on it, so an importer whose supplier was not on Snowkap could record nothing. Two walls: (1) provenance must match capability -- `OPERATOR_PROVIDED` requires `PRODUCER_OPERATOR`, `IMPORTER_ENTERED` requires `IMPORTER_DECLARANT`, an org holding both may claim either, and provenance is immutable once written (rewriting it would turn transcribed data into operator-attested data with one UPDATE); (2) `ActualEmissionSnapshot.record_provenance` is validated against the installation's own, without which the field would be decorative and a raw write could set it to anything. Nothing about the verification lifecycle, evidence completeness or ACTUAL eligibility is relaxed. Body carried across verbatim from `20260902140000` (v10). |
| `20260903130000` | `p14_audit_catalog_invitation_events.sql` | **Audit gap.** `audit_events` recorded every change to a membership that already existed and nothing about how one came to exist, so the trail could be read end to end without ever showing how an administrator became one -- inviting is the only way into an organization and an invitation carries a ROLE. Adds `membership.invitation_created` and `membership.invitation_revoked`, reusing `aggregate_type` `MEMBERSHIP` with the invitation's id exactly as `accept_organization_invitation` already does, so no `aggregate_type` CHECK changes and the whole invite -> accept -> role-change -> remove story stays one queryable aggregate. The catalog is an allowlist, so the policy is re-created whole; every existing value carried across verbatim. |
| `20260903140000` | `p14_emission_data_verification_downgrade_gate.sql` | **Cross-org evidence strip.** `20260829560000` makes the evidence behind a VERIFIED record immutable, and `determine-from-actual-data.ts` relies on that when freezing an evidence set into an importer's snapshot -- but the verification gate only ever fired on transitions INTO VERIFIED/REJECTED. An ADMIN could walk an ACTIVE record back to VERIFICATION_PENDING, remove a file, and verify it again; `verifier_user_id` survives untouched, so nothing in the row records it. The importer's frozen determination then cites documents that no longer exist and the v10 validator refuses to re-save that line, with an error explaining nothing. An ACTIVE, VERIFIED record can no longer leave VERIFIED. Narrow: DRAFT records move freely, and DISCARD/supersession change `status` rather than `verification_status`, so a verified record is never trapped. Body carried across verbatim from `20260829480000`. |
