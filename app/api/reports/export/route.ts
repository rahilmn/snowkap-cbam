...
export async function GET(
  ...
  notes.addRows(
    [
      ...
      { note: "Calculation currency: CURRENT means the figure is the calculation of the determination this line carries now. STALE means the line was re-determined after it was calculated, so the stored figure describes a determination the line no longer carries -- a state the filing gate refuses, and one whose figure is excluded from this period's totals. DATASET_SUPERSEDED means the figure is a current calculation of a default value resolved against a regulatory dataset that has since been corrected -- a different state the filing gate also refuses; unlike STALE, this figure is still included in this period's totals (see the report's own dataset-superseded list). ENGINE_VERSION_OUTDATED means the figure is a current calculation, but it was produced by a version of the calculation engine that is no longer current -- another state the filing gate also refuses; like DATASET_SUPERSEDED, this figure is still included in this period's totals (see the report's own engine-outdated list). NOT_CALCULATED means no calculation exists for the line." },
